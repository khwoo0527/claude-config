---
paths:
  - "docs/sprint/**"
  - "ROADMAP.md"
  - "deploy.md"
---

# 스프린트/핫픽스 워크플로우 규칙

> 이 파일은 기술 스택에 무관하게 적용되는 범용 워크플로우입니다.
> 프로젝트별 기술 스택, 빌드 명령, 브랜치 전략은 `/CLAUDE.md`에 정의합니다.
> 에이전트는 반드시 `/CLAUDE.md`의 `tech_stack`을 확인하고, 해당하는 `.claude/rules/tech/{tech}.md`를 로드하여 기술 스택 전문가로 동작합니다.

---

## 기술 스택 연동 패턴

모든 에이전트와 워크플로우는 아래 순서로 프로젝트 컨텍스트를 로드합니다:

```
1. /CLAUDE.md 읽기 → 기술 스택, 빌드 명령, 폴더 구조, 브랜치 전략 파악
2. .claude/rules/ 스캔 → CLAUDE.md의 tech_stack에 해당하는 규칙 파일만 로드
3. 프로젝트 문서 읽기 → ROADMAP.md, docs/phase/, docs/sprint/ 등
```

**규칙 파일 로드 원칙:**
- CLAUDE.md에 명시된 기술 스택에 **정확히 매칭되는** rules/tech/{tech}.md만 로드합니다.
- 관련 없는 기술 스택의 rules 파일은 **절대 로드하지 않습니다.** (예: Python 프로젝트에서 rules/tech/csharp.md 로드 금지)
- `rules/workflow/sprint-workflow.md`, `rules/workflow/notion.md` 등 기술 스택이 아닌 워크플로우/연동 규칙은 해당 시 로드합니다.

**예시:**
- CLAUDE.md에 `tech_stack: C#, WinForms` → `rules/tech/csharp.md` 로드
- CLAUDE.md에 `tech_stack: Python, FastAPI` → `rules/tech/python.md` 로드
- CLAUDE.md에 `tech_stack: TypeScript, React` → `rules/tech/react.md` + `rules/tech/typescript.md` 로드
- Notion 문서 작성이 필요하면 → `rules/workflow/notion.md` 추가 로드

**해당 기술 스택의 rules/tech/{tech}.md가 없는 경우:**
- 에이전트는 작업을 시작하기 전에 **사용자에게 rules/tech/{tech}.md 생성을 먼저 제안**합니다.
- 사용자가 동의하면 **`.claude/rules/workflow/rules-guide.md`를 참조하여** 해당 기술 스택의 시니어 수준 규칙 파일(언어 + 프레임워크 + 실전 패턴)을 생성한 뒤 작업을 진행합니다.
- rules-guide.md의 작성 원칙, 템플릿 구조, 품질 체크리스트를 반드시 따릅니다.
- rules 파일 없이 진행하면 기술 스택 전문성이 보장되지 않으므로, **생성을 강력히 권장**합니다.

이를 통해 코드 리뷰, 빌드 검증, 코드 품질 기준이 **프로젝트의 기술 스택에 특화**됩니다.

---

## Sprint 프로세스

### 1. 계획 (sprint-planner agent)
- ROADMAP.md + 코드베이스 분석 → `docs/sprint/sprint{N}.md` 생성
- sprint{N}.md는 **실행 명세서**: Task별 파일 경로, Step, 검증 명령, 커밋 메시지 포함
- CLAUDE.md의 기술 스택에 맞는 빌드/검증 명령을 sprint 문서에 반영
- 해당 기술 스택의 베스트 프랙티스(rules/tech/{tech}.md)를 기반으로 Task 설계
- 사용자가 검토/승인한 후 구현 단계로 진행

### 2. 구현 (sprint{N}.md 기준)
- CLAUDE.md에 정의된 브랜치 전략에 따라 `sprint{N}` 브랜치 생성
- **sprint{N}.md를 먼저 읽고** 실행 플랜과 Task 목록을 파악
- 각 Task의 `skill:` 헤더에 명시된 스킬을 Skill 도구로 로드
- 실행 플랜의 Phase 순서대로 Task 실행:
  - **병렬 가능 Phase**: 사용자 요청 시 팀으로 병렬 실행 가능
  - 각 Task의 Step을 따름 (skill별 실행 전략에 따라)
  - CLAUDE.md에 정의된 빌드/검증 명령으로 결과 확인
  - **simplify** 스킬로 코드 정리 후 커밋
  - 명시된 커밋 메시지로 커밋
- 최종 검증 시 **verification-before-completion** 스킬 적용
- 계획과 다른 결정이 필요하면 사용자에게 확인
- worktree 사용 금지

#### 🔴 에러/버그 수정 원칙 (구현 중 적용)

문제가 발생하면 **증상 해결(땜빵)이 아닌 근본 원인을 먼저 진단**한다.

```
에러 발생 → 1) 왜 이 에러가 발생하는가 (근본 원인 분석)
          → 2) 이 구조 자체가 잘못된 것인가? 설정이 잘못된 것인가?
          → 3) 구조 변경이 필요하면 구조를 바꾼다 (땜빵 금지)
          → 4) 수정 후 같은 시나리오에서 재발하지 않는지 검증
```

**나쁜 예**: API 호출 시 403 에러 → 인증 체크 제거(땜빵) → 관리자 토큰 하드코딩(또 땜빵) → 결국 권한 로직 자체를 수정(정답)
**좋은 예**: API 호출 시 403 에러 → 권한 검증 로직 점검 → 역할(role) 매핑 누락 발견 → 권한 로직 자체를 올바르게 수정

#### 🔴 우회/핵 금지 원칙

보안 메커니즘(접근 제어, CORS, 인증, 권한)이 문제를 일으키면 **메커니즘을 우회하지 않고 올바르게 설정**한다.

| 우회 패턴 (금지) | 왜 위험한가 | 정석 해결 |
|-----------------|-----------|----------|
| 관리자 API에 인증/인가 생략 | 누구나 관리자 기능 사용 가능 → 데이터 유출/파괴 | 모든 엔드포인트에 인증 + 역할 기반 인가 적용 |
| 에러 발생 시 try-catch로 삼키기 | 근본 원인 파악 불가, 데이터 무결성 훼손 | 에러를 적절히 전파하고 원인 수정 |
| CORS를 `*`로 열기 | 모든 도메인에서 API 접근 가능 | 허용 도메인만 명시 |
| 인증 체크를 클라이언트에서만 | 서버 우회로 무인증 접근 | 서버/DB 레벨에서 인증 강제 |

**핵심**: "일단 동작하게 하고 나중에 고치자"는 기술 부채가 아니라 **보안 시한폭탄**이다.
AI가 해당 기술의 전문가로서 **"이게 정석인지"를 먼저 판단**하고 가이드해야 한다.

### 3. 완료 기준 (Sprint 완료 = 실환경 동작 확인)

> **🔴 빌드 통과, 테스트 통과만으로 Sprint 완료가 아니다.**
> **실제 배포 환경(스테이징 서버, 프리뷰 환경, 실기기 등)에서 핵심 기능이 동작해야 완료다.**

Sprint "완료" 선언 전 반드시 확인:

| 확인 항목 | 설명 |
|----------|------|
| **빌드 통과** | 컴파일, lint, 테스트 — 기본 문턱 |
| **실환경 배포** | 스테이징/프리뷰 환경에 실제 배포 |
| **핵심 기능 동작** | Sprint 목표 기능이 실환경에서 정상 동작 |
| **데이터 흐름 검증** | DB 쿼리, API 호출, 인증이 실환경에서 정상 |
| **크로스 플랫폼** | 웹/모바일 등 타겟 플랫폼에서 모두 테스트 |

**"빌드 되니까 완료" → ❌**
**"실환경에서 동작 확인했으니 완료" → ✅**

### 4. 마무리 (sprint-close agent)
- sprint-close agent → ROADMAP 업데이트 + develop PR 생성 + 문서 정리
- 코드 리뷰와 자동 검증은 수행하지 않음

### 5. 리뷰 (Sprint Retrospective)
- Sprint 완료 후 **반드시 실행 → 테스트 → 리뷰** 단계를 거친다
- 사용자와 함께 테스트 가이드 기반으로 전체 플로우를 확인한다
- 코드 리뷰 시 해당 기술 스택의 rules/tech/{tech}.md 기준으로 전문적 리뷰 수행
- 사용자가 "확인 완료"할 때까지 다음 Sprint로 넘어가지 않는다 (감독 레벨 L0~L2 기준. L3 은 [`supervision.md`](supervision.md) 따라 Phase 리뷰가 게이트)

#### 리뷰 시 발견된 이슈 → Backlog 분류
| 우선순위 | 의미 | 처리 |
|---------|------|------|
| 🔴 Critical | 현재 Sprint에서 반드시 수정 | 즉시 수정 |
| 🟠 High | 다음 Sprint에 반드시 포함 | Sprint Backlog에 추가 |
| 🟡 Normal | 가까운 Sprint에 포함 | Product Backlog에 추가 |
| 🟢 Low | 여유될 때 처리 | Product Backlog에 추가 |
| 🔧 Tech Debt | 기술 부채 (동작하지만 개선 필요) | Product Backlog에 추가 |

#### Backlog 관리
- 파일: `docs/backlog.md`
- Sprint 리뷰에서 발견된 이슈를 기록한다
- Sprint Planning 시 Backlog에서 다음 Sprint에 포함할 항목을 선별한다 (Refinement)
- 완료된 항목은 `[x]`로 체크하고, 어느 Sprint에서 해결했는지 기록한다

### 6. Phase 리뷰
- Phase(Sprint 여러 개 묶음) 완료 시 **전체 플로우 통합 점검**
- Phase 목표 달성 여부 확인
- 다음 Phase 방향 재확인 및 조정
- Phase 리뷰에서 발견된 이슈도 Backlog에 기록

### 7. 배포
- QA 후 deploy-prod agent → 빌드 생성 + 배포 준비
- CLAUDE.md에 정의된 배포 절차에 따라 진행
- 메인 브랜치 병합은 사용자가 직접 수행

---

## Hotfix 프로세스
1. 메인 브랜치 기반 `hotfix/{설명}` 브랜치 생성
2. 핫픽스 구현 후 hotfix-close agent → **develop** PR 생성
3. develop에서 QA 후 메인 브랜치 병합은 사용자가 직접 수행
> 메인 브랜치에 직접 PR을 생성하거나 병합하지 않습니다.

## Hotfix vs Sprint 판단 기준
- Hotfix: 프로덕션 장애, 파일 3개 이하, 코드 50줄 이하, 스키마 변경/새 의존성 없음
- Sprint: 이외 모든 작업

---

## 예외 상황 대응

### 스프린트 중간 방향 전환 / 요구사항 변경
1. **경미한 변경** (Task 수정/추가 수준): sprint{N}.md를 직접 수정하고 계속 진행
2. **중대한 변경** (Sprint 목표 자체가 바뀜): 현재 Sprint를 중단하고 아래 절차를 따름
   - 진행된 작업을 커밋 (미완성이라도 `wip:` 접두어로 커밋)
   - sprint{N}.md에 중단 사유와 진행 상태를 기록
   - 필요 시 Phase 재계획 (phase-planner) 또는 새 Sprint 계획 (sprint-planner)
3. **Phase 재계획이 필요한 경우**: 아키텍처 변경, 기술 스택 변경, 핵심 요구사항 대폭 수정 시

### 롤백 (배포 후 문제 발생)
1. **경미한 버그**: Hotfix 프로세스로 대응
2. **심각한 장애 (즉시 롤백 필요)**:
   - 메인 브랜치를 이전 안정 버전으로 되돌림: `git revert` 또는 `git reset`
   - 롤백 후 deploy.md에 롤백 기록 작성
   - 원인 분석 후 Hotfix 또는 Sprint로 근본 수정
3. **롤백 판단 기준**: 사용자 영향도가 높고, 핫픽스로 즉시 해결 불가능한 경우

---

## 코드 품질 기준

코드 리뷰와 구현 시 아래 두 레이어로 품질을 보장합니다:

### Layer 1: 범용 소프트웨어 공학 원칙 (모든 프로젝트 공통)
- SOLID 원칙 준수
- DRY (Don't Repeat Yourself) — 3회 이상 반복 시 추출
- KISS (Keep It Simple, Stupid) — 불필요한 추상화 지양
- 보안: OWASP Top 10 기준 취약점 방지
- 성능: 불필요한 연산, 메모리 낭비 주의
- 테스트: 프로젝트에 테스트 전략이 있으면 준수
- **근본 원인 우선**: 에러 발생 시 증상 해결이 아닌 근본 원인 분석 → 구조 변경이 필요하면 구조를 변경
- **우회 금지**: 보안/인증 메커니즘을 우회하지 않는다 — 메커니즘 자체를 올바르게 설정
- **실환경 검증**: 빌드 통과 ≠ 동작 확인. 실제 환경에서 기능이 동작해야 "완료"

### 재활용성 & 모듈화 원칙
구현 시 다음 Sprint/Phase에서 재활용 가능한 구조로 설계한다.

| 원칙 | 설명 |
|------|------|
| **공용 컴포넌트 우선** | 2개 이상 화면에서 쓸 것 같으면 공용(shared)으로 만든다 |
| **데이터 상수 분리** | 하드코딩 대신 상수 파일로 분리한다 |
| **비즈니스 로직 분리** | 화면에 직접 넣지 않고 커스텀 훅/서비스 함수로 분리한다 |
| **서비스 레이어 분리** | 외부 API/DB 호출은 서비스 함수로 분리한다 |

**판단 기준**: "이 코드가 다른 화면/기능에서도 필요할까?" → Yes면 공용으로, No면 해당 기능 폴더에.
**타이밍**: 처음부터 과도한 추상화는 지양하되, 두 번째 사용처가 생기는 시점에 반드시 공용화한다.

### 보안 원칙 (작업 중 상시 적용)

보안은 리뷰 때 한 번 보는 것이 아니라 **매 작업마다 확인**한다.
기술별 구체적 보안 규칙은 `rules/tech/{tech}.md`의 보안 섹션을 따른다.

| 원칙 | 설명 |
|------|------|
| **접근 제어 필수** | DB 테이블, API 엔드포인트, 파일 저장소 — 생성 시 반드시 접근 제어 정책을 함께 작성 |
| **입력 검증 이중 방어** | 클라이언트 검증은 UX용, 서버/DB 검증이 진짜 방어선 — 둘 다 있어야 한다 |
| **민감 정보 분류** | 환경변수, 토큰, 개인정보 — 공개/민감을 분류하고 민감 정보는 클라이언트에 노출 금지 |
| **인증/권한 확인** | 새 화면/기능 추가 시 "인증 필요 여부"와 "누가 접근 가능한가"를 반드시 확인 |
| **의존성 보안** | 정기적으로 패키지 취약점 점검, 알려진 취약 버전 사용 금지 |
| **로깅 시 마스킹** | 로그에 토큰, 비밀번호, 개인정보가 노출되지 않도록 마스킹 |
| **즉시 대응** | 보안 문제 발견 시 다음 Sprint로 미루지 않는다 — 즉시 사용자에게 알리고 수정 |

**Sprint 리뷰 시 보안 점검 항목:**
- [ ] 새로 추가된 DB 테이블/엔드포인트에 접근 제어가 적용되었는가?
- [ ] 사용자 입력이 검증 없이 DB/화면에 반영되는 곳은 없는가?
- [ ] 민감 정보가 클라이언트 코드나 로그에 노출되지 않는가?
- [ ] 환경변수가 올바르게 분류되고 gitignore에 반영되었는가?

### Layer 2: 기술 스택 전문 규칙 (rules/tech/{tech}.md에서 로드)
- 해당 언어/프레임워크의 베스트 프랙티스
- 네이밍 컨벤션, 코드 구조, 패턴
- 언어별 보안 취약점 (SQL Injection, XSS 등)
- 빌드/패키지 관리 규칙
- 프레임워크별 안티패턴

---

## 문서 구조
- 실행 명세서: `docs/sprint/sprint{N}.md`
- 첨부: `docs/sprint/sprint{N}/`
- Phase 계획: `docs/phase/phase{N}.md`
- 배포 기록: `deploy.md`
- 배포 히스토리: `docs/deploy-history/`

## 체크리스트 형식
- 완료: `- [x] 항목 내용`
- 미완료: `- [ ] 항목 내용`

---

## 새 프로젝트 초기화

새 프로젝트 시작 책임/절차는 [`project-init` 에이전트](../../agents/project-init.md) 가 단일 진실 원천이다. 본 워크플로우는 그 정책을 따른다.

**진입 신호** (다음 시 사용자에게 새 프로젝트 여부 확인 후 `project-init` 호출):
- CLAUDE.md 가 없거나 현재 코드베이스와 agent-memory 내용이 맞지 않을 때
- 사용자가 새 프로젝트를 시작하는 뉘앙스 (직접적이 아니더라도)
- 이전 프로젝트와 기술 스택이 다른 작업을 요청받을 때

> **원칙**: .claude 폴더는 범용 프레임워크. 프로젝트 종속 데이터는 `agent-memory/` 와 프로젝트 루트 (CLAUDE.md, ROADMAP, deploy.md 등) 에만 존재. 새 프로젝트 시 `project-init` 의 정리 절차로 리셋.
