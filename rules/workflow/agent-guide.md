...
color: blue | green | red | ...  # 단순 시각 구분 (의미 정책 X — 가독성용)
# skills: [...]                  # (선택) 사용할 Skill 명시. 없으면 생략
# maxTurns: 50                   # (선택) 긴 작업 한정. 기본값 사용 시 생략
---
```

> **`color` 정책**: 단순 시각 구분 — 의미 정책 X (예: red = 위험 작업, green = 부트스트랩 같은 의무 매핑 없음). 같은 색이 여러 에이전트에 쓰여도 OK. 패턴 분류 (자기 캐시 보유 / 외부 시스템 연동 등) 는 색 아닌 § 7 표 참조.
...
| 섹션 | 형태 | 비고 |
|---|---|---|
| 역할 정의 | 첫 문단 ("당신은 **{역할}**입니다") | |
| 핵심 원칙 또는 동등 도메인 변형 | `## 핵심 원칙` 또는 도메인 명칭 (예: notion-writer 의 `## 1. 디자인 철학`, project-init 의 `## 핵심 원칙`) | 본질이 같으면 명칭 자유 |
| 작업 절차 | `## 작업 절차` 또는 `## 작업 절차 (필수)` | 단계 수는 도메인별 |
| 발견된 정보 분기 기록 | `## 발견된 정보 분기 기록` 또는 본문 내 박스 | § 7 패턴별 변형 허용 — 자기 캐시 보유는 단계로, 자기 캐시 없음은 박스 1줄 가능 |
| 에러 처리 | `## 에러 처리` | |

### 선택 섹션 (해당 시)

- **기술 스택 적응 / 전문가 모드 박스** — 첫 문단 뒤
- **외부 서비스 위치 정의 박스** — 첫 문단 뒤 (외부 시스템 연동 에이전트 한정 — § 3 참조)
- **입력** — 사용자/메인 Claude 가 제공해야 할 입력 명세 (Sprint 등 명세 필수 에이전트)
- **사용자 다음 단계 안내** — 작업 완료 시 (해당 시)
...
## 핵심 원칙   ← 도메인 변형 OK (예: "디자인 철학" / "역할 및 책임")
...
### {N단계}: 발견된 정보 분기 기록   ← 자기 캐시 없음 패턴은 박스 1줄로 단순화 가능
...
> **§ 7 패턴별 변형**: 자기 캐시 없음 패턴 (`sprint-close`, `hotfix-close`, `deploy-prod`, `project-init`) 은 분기 기록을 단계 형태 또는 작업 절차 끝 박스 1줄로 단순화 가능 (예: `> 발견된 정보 분기 기록: 자기 캐시 없음 — 발견 노하우는 rules/tech 또는 다른 에이전트 캐시...`).
...
## 4.5 실행 방식 — 인터뷰형 vs 일괄형 (단일 진실 원천)

> **핵심 제약**: Task tool 서브에이전트는 **실행 중간에 사용자와 대화할 수 없다** (질문은 최종 메시지로만 반환됨).

| 유형 | 특징 | 실행 방식 |
|---|---|---|
| **인터뷰형** (절차에 사용자 질문/중간 확인 포함) | `project-init`, `phase-planner`(브레인스토밍 구간), `prd-to-roadmap`(요구사항 모호 시 인터뷰 모드 전환 경로 보유), 요구사항 인터뷰 | **메인 스레드가 에이전트 문서를 Read 하여 인라인 수행** — 이때도 문서의 단계·품질 기준·출력 형식을 그대로 따른다 (`feedback_principles` "에이전트 절차 준수" 연동) |
| **일괄형** (입력만 받으면 완주 가능) | `sprint-planner`(L2), `sprint-close`, `sprint-review`, `deploy-prod`, `hotfix-close`, `notion-writer` | 서브에이전트 스폰 가능 (컨텍스트 격리 이점) — 물론 인라인 수행도 유효 |

> 일괄형이라도 절차에 조건부 컨펌(예: rules/tech 부재 시 생성 제안)이 있으면, 스폰 시 해당 지점에서 "질문 반환 후 종료" 될 수 있다 — L2+/위임 상황에서는 합리적 기본값으로 진행 + 사후 보고가 원칙 (blanket_authorization 연동).

- 새 에이전트 작성 시 어느 유형인지 판단하고, 인터뷰형이면 본문 상단에 실행 방식 박스 1줄을 명시한다.
- 인터뷰형을 스폰하면: 에이전트가 질문만 반환하고 종료 → 재스폰 시 컨텍스트 소실 → 사용자 혼란. 금지.
